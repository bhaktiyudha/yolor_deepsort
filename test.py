...
class YoLov5TRT(object):
    ...
    def infer(self, input_image_path):
        ...
        t1 = time.time()
        bboxes = self.post_process_new(
            output, origin_h, origin_w)
        # Do Deepsort
        if len(bboxes) > 0:
            list_bboxs = tracker.update(bboxes, image_raw)
            output_image_frame = tracker.draw_bboxes(image_raw, list_bboxs, line_thickness=None)
        else:
            output_image_frame = image_raw

        fps = (0.0 + (1. / (time.time() - t1))) / 2
        ...
        print('time->{:.2f}ms, saving into output/'.format((end-start) * 1000))
    ...
    def preprocess_image(self, raw_bgr_image):
        ...
        if r_h > r_w:
            ...
            th = int(r_w * h)
            tx = 0
            ty = int((self.input_h - th) / 2)
        else:
            tw = int(r_h * w)
            th = self.input_h
            tx = int((self.input_w - tw) / 2)
            ty = 0
...
class myThread(threading.Thread):
    def __init__(self, yolov5_wrapper, image,video_writer):
        threading.Thread.__init__(self)
    ...
    def run(self):
        raw_image, use_time = self.yolov5_wrapper.infer(self.image)
        print('time->{:.2f}ms'.format(use_time * 1000))
        self.video_writer.write(raw_image)
...
if __name__ == '__main__':
    ...
    videoWriter = cv2.VideoWriter("result.mp4", cv2.VideoWriter_fourcc(*'mp4v'), fps, (frame_width, frame_height))

    while True:
        ret,img = video.read()
        if img is not None:
        # create a new thread to do inference
            thread1 = myThread(yolov5_wrapper, img,videoWriter)
            thread1.start()
            thread1.join()
        else:
            break
